    ...
    public function uploadExcel($date ='',$bumen_id ='')
    {
        ...
        foreach ($dept_id as $k1 => $v1)
        {
            ...
            if (!count($cx))  //判断此循环下的部门是否已导入
            {
                ...
                //设置excel标题
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1','部门');            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1','业务类型');            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1','说明');            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1','防盗门');            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1','三代机');$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1','地面波');
                $i =2;
...
//				$xls = json_decode($revurl,true);
			    ...
			    if(!count($cxo))
				{
					...
					{                //当前部门的文件下载地址存入数据库
						$sql = "insert into xsrb_excel(`dept_id`,`biao`,`date`,`url`) values(".$v1['id'].",'qtsrmx',$date,'$keys')";
						M()->execute($sql);
					}				
				}					
				$ret = -1;
