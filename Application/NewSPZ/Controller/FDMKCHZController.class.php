...
require_once "Application/Home/Controller/XSRBUtil.php";
//防盗门库存汇总表
class FDMKCHZController extends RestController{
    ...
    public function search($token='',$startDate='',$endDate='',$page=1,$pageSize=1,$deptId='',$pId='',$flag=false){
        ...
        $json_head=[
            ...
            [
                ["value"=>"","rowspan"=>1,"colspan"=>1],["value"=>"","rowspan"=>1,"colspan"=>1],
                ["value"=>"收入","rowspan"=>1,"colspan"=>2],["value"=>"支出","rowspan"=>1,"colspan"=>8],
                ["value"=>"有效结存","rowspan"=>2,"colspan"=>1],["value"=>"无效调拨收入","rowspan"=>2,"colspan"=>1],
                ["value"=>"支出","rowspan"=>1,"colspan"=>3],["value"=>"无效结存","rowspan"=>2,"colspan"=>1],
            ],
            //第四行
            [
                ["value"=>"","rowspan"=>1,"colspan"=>1],["value"=>"","rowspan"=>1,"colspan"=>1],
                ["value"=>"有效调拨收入","rowspan"=>1,"colspan"=>1],
                ["value"=>"其他收入","rowspan"=>1,"colspan"=>1],["value"=>"直发销售支出","rowspan"=>1,"colspan"=>1],
                ["value"=>"库房销售支出","rowspan"=>1,"colspan"=>1],["value"=>"有效调拨支出","rowspan"=>1,"colspan"=>1],
                ["value"=>"暂存商品","rowspan"=>1,"colspan"=>1],["value"=>"铺货商品","rowspan"=>1,"colspan"=>1],
                ["value"=>"送货支出","rowspan"=>1,"colspan"=>1],["value"=>"报废支出","rowspan"=>1,"colspan"=>1],
                ["value"=>"有效其他支出","rowspan"=>1,"colspan"=>1],
                ["value"=>"无效报废支出","rowspan"=>1,"colspan"=>1],["value"=>"无效调拨支出","rowspan"=>1,"colspan"=>1],
                ["value"=>"无效其他支出","rowspan"=>1,"colspan"=>1],
            ],
            //第五行
            [
        ...
        foreach ($depts as $obj_dept){
            ...
            $dept_data=array();
            //查询按非结存数据
            $sql_data="select 1 as sort, 'bumentj'as col1,zhizaobm as col2,t1.type,sum(dbsr) as dbsr ,sum(zcsr) as zcsr, sum(phsr) as phsr,sum(qtsr) as qtsr, sum(zfxszc) as zfxszc, 
                sum(kfxszc) as kfxszc, sum(dbzc) as dbzc, sum(zczc) as zczc,sum(phzc) as phzc,sum(shzc) as shzc,sum(bfzc) as bfzc,sum(qtzc) as qtzc  
                from new_spzmx t1,new_spzmxqc t2 where t1.product_md5=t2.product_md5 and t1.dept='$obj_deptId' and date='$startDate' and `dangci` not in ('锁体','锁芯','锁把','配件','门套') and zhizaobm not in ('舍零','返利','送货运费收入') group by zhizaobm,t1.type 
                UNION
                select 2 as sort ,'dangcitj'as col1,dangci as col2,t1.type,sum(dbsr) as dbsr,sum(zcsr) as zcsr, sum(phsr) as phsr,sum(qtsr) as qtsr, sum(zfxszc) as zfxszc, 
                sum(kfxszc) as kfxszc, sum(dbzc) as dbzc, sum(zczc) as zczc,sum(phzc) as phzc,sum(shzc) as shzc,sum(bfzc) as bfzc,sum(qtzc) as qtzc 
                from new_spzmx t1,new_spzmxqc t2 where t1.product_md5=t2.product_md5 and t1.dept='$obj_deptId' and date='$startDate'  and zhizaobm not in ('舍零','返利','送货运费收入') group by dangci,t1.type  
		...
		(
			...
			FROM
				(
				#查询按部门统计的数据
				SELECT 1 AS sort,'bumentj' AS col1,zhizaobm AS col2,type,sum(qichusl)+IFNULL(
	              #查询月结存(不包含期初)
	              (select sum(jiecun) from new_spzmx t1,new_spzmxqc t2 where t1.dept='$obj_deptId' and t1.date between '$firstDay' and '$endDate' and t1.product_md5=t2.product_md5 and t2.zhizaobm=out_qc.zhizaobm and t1.type=out_qc.type
                  #内控不包括字段(结存不包含期初)
	               and t2.`dangci` not in ('锁体','锁芯','锁把','配件','门套') and t2.zhizaobm not in ('舍零','返利','送货运费收入')  ),0 ) AS jiecun
                FROM new_spzmxqc out_qc WHERE dept = '$obj_deptId' AND `month`= '$month' 
                 #内控不包括字段(期初)
	               and `dangci` not in ('锁体','锁芯','锁把','配件','门套') and zhizaobm not in ('舍零','返利','送货运费收入')
                GROUP BY zhizaobm,type
			  UNION
				#查询按档次统计的数据
				SELECT 2 AS sort,'dangcitj' AS col1,dangci AS col2,type,sum(qichusl)+IFNULL(
	              #查询月结存(不包含期初)
	              (select sum(jiecun) from new_spzmx t1,new_spzmxqc t2 where t1.dept='$obj_deptId' and t1.date between '$firstDay' and '$endDate' and t1.product_md5=t2.product_md5 and t2.dangci=out_qc.dangci and t1.type=out_qc.type
                 #内控不包括字段
	               and t2.zhizaobm not in ('舍零','返利','送货运费收入')  ),0 ) AS jiecun
                FROM new_spzmxqc out_qc WHERE dept = '$obj_deptId' AND `month`= '$month' 
                #内控不包括字段
	               and zhizaobm not in ('舍零','返利','送货运费收入')
                GROUP BY dangci,type
					) AS t2
			WHERE
			t2.sort = new_spzmx_neikong.sort
			AND t2.col1 = new_spzmx_neikong.col1
			AND t2.col2 = new_spzmx_neikong.col2
			AND t2.type = new_spzmx_neikong.type
		),
		0
	) AS jiecun
    FROM
	new_spzmx_neikong ORDER BY sort,id,col2,type desc";
            ...
            foreach ($temp_array as $key=>$row){
                ...
                $temp_ret[$key][2]=["rowspan"=>1,"colspan"=>1,"value"=>empty($row['qtsr'])?0:$row['qtsr']];//其他收入
                $temp_ret[$key][3]=["rowspan"=>1,"colspan"=>1,"value"=>empty($row['zfxszc'])?0:$row['zfxszc']];//直发销售支出
                $temp_ret[$key][4]=["rowspan"=>1,"colspan"=>1,"value"=>empty($row['kfxszc'])?0:$row['kfxszc']];//库房销售支出
                $temp_ret[$key][5]=["rowspan"=>1,"colspan"=>1,"value"=>empty($row['dbzc'])?0:$row['dbzc']];//调拨支出
                $zcsp=empty($row['zczc']-$row['zcsr'])?0:$row['zczc']-$row['zcsr'];//支出-收入??
                $temp_ret[$key][6]=["rowspan"=>1,"colspan"=>1,"value"=>$zcsp];//暂存商品
                $phsp=empty($row['phzc']-$row['phsr'])?0:$row['phzc']-$row['phsr'];//支出-收入??
                $temp_ret[$key][7]=["rowspan"=>1,"colspan"=>1,"value"=>$phsp];//铺货商品
                $temp_ret[$key][8]=["rowspan"=>1,"colspan"=>1,"value"=>empty($row['shzc'])?0:$row['shzc']];//送货支出
                $temp_ret[$key][9]=["rowspan"=>1,"colspan"=>1,"value"=>empty($row['bfzc'])?0:$row['bfzc']];//报废支出
                ...
                $wx_qtzc=$row['wx_zfxszc']+$row['wx_kfxszc']+$row['wx_zczc']+$row['wx_phzc']+$row['wx_shzc']+$row['wx_shzc'];
                ...
                switch($row['col1']){
                    ...
                    case "bumentj":
                        ...
                        $json_head[4][2]['value']+=empty($row['qtsr'])?0:$row['qtsr'];//其他收入
                        $json_head[4][3]['value']+=empty($row['zfxszc'])?0:$row['zfxszc'];//直发销售支出
                        $json_head[4][4]['value']+=empty($row['kfxszc'])?0:$row['kfxszc'];//库房销售支出
                        $json_head[4][5]['value']+=empty($row['dbzc'])?0:$row['dbzc'];//调拨支出
                        $json_head[4][6]['value']+=$zcsp;//暂存商品
                        $json_head[4][7]['value']+=$phsp;//铺货商品
                        $json_head[4][8]['value']+=empty($row['shzc'])?0:$row['shzc'];//送货支出
                        $json_head[4][9]['value']+=empty($row['bfzc'])?0:$row['bfzc'];//报废支出
            ...
            if(!empty($temp_ret)){
                ...
                array_unshift($temp_ret[$bm_rowspan],array("value"=>"按档次统计","colspan"=>1,"rowspan"=>$dl_rowspan));
            }
    ...
    public function toExcel($printData,$dept='',$date='',$flag=false){
            ...
            for($j=$fromDept;$j<$destDept;$j++){
                ...
                foreach($deptVal['head'] as $headRow=>$headVal){
                    ...
                    if($headRow==1){
                        ...
                        $objPHPExcel->getActiveSheet()->getStyle("$colPos:$det")->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                        //$rowPos++;
                    }
                    //第三行
                    if($headRow==2){
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue($colPos."3","有效结存");
                        $objPHPExcel->getActiveSheet()->mergeCells($colPos."3:".$colPos."4");
                        $colPos=\PHPExcel_Cell::stringFromColumnIndex($colPosInt+11);
                        $objPHPExcel->getActiveSheet()->setCellValue($colPos."3","无效调拨收入");
                        $objPHPExcel->getActiveSheet()->mergeCells($colPos."3:".$colPos."4");
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValue($colPos."3","无效结存");
                        $objPHPExcel->getActiveSheet()->mergeCells($colPos."3:".$colPos."4");
                    }
                    //第四行
                    if($headRow==3){
        ...
        if($flag){
            ...
            return \XSRBUtil::uploadExcel($dept_id, $objWriter,'NEW_FDMKCHZ',$date);
        }else
            return $objWriter;
    ...
    public function printExcel($token='',$startDate='',$endDate=''){
        ...
        $filename="NEW_FDMKCHZ-$startDate-$dept_id.xls";
    ...
    public function downloadExcel($token='',$startDate='',$endDate=''){
        ...
        if($qt1==0||$dept_id==1){
            //查询上传excel的地址
            $sql_addr="select url from xsrb_excel where dept_id='$dept_id' and date='$startDate' and biao='NEW_FDMKCHZ' ";
            $ret_addr=M()->query($sql_addr);
            if($_SERVER['SERVER_NAME'] =='172.16.10.252' && count($ret_addr))
            {
                $downloadUrl ="http://172.16.10.252/files/"."NEW_FDMKCHZ"."-".$startDate."-".$dept_id.".xls" ;
            }
            else
            {
