...
class FDMKCBMXController extends RestController
{
    ...
    public function search($token ='',$date ='',$page ='',$pagesize ='',$type ='',$id ='')
    {
        ...
        $banben = checkbanben(str_replace('\\','/',realpath(__DIR__)."/tempJson"),date('Ym',strtotime($date)));;
        $handle=fopen(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/mx_json1.txt"),'r');
        $mx_json1=fread($handle, filesize(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/mx_json1.txt")));
        ...
        }elseif ($type =='excel')
        {
            ...
            {
                //通过redis的数据来生成报表
                $fdm_json = $redis->get("report-".$vk['id']."-$date-FDMKC");
                if ($fdm_json !='')
                {
    ...
    public function uploadExcel($date ='',$bumen_id ='')
    {
        ...
        $banben =  checkbanben(str_replace('\\','/',realpath(__DIR__)."/tempJson"),date('Ym',strtotime($date)));
		$handle=fopen(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/mx_json2.txt"),'r');
		$mx_json2=fread($handle, filesize(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/mx_json2.txt")));
		fclose($handle);
        foreach ($dept_id as $kde => $vde)
        {
            ...
            if (!count($cx))
            {
                ...
                unset($temp);
                $objPHPExcel->getActiveSheet()->freezePane('A4');       //冻结单元格
                //生成xls文件,保存在当前项目目录下
...
//                 $keys = '/var/www/excel/'.$vde['id'].'-fdmkcbmx-'.$date.'.xls';
                ...
                if ($bumen_id !='')
                {
                    ...
                    $objWriter1->save('php://output'); //文件通过浏览器下载
                    return;
                }
...
//				$xls = json_decode($revurl,true);
                ...
                if(!count($cxo))
				{
					...
					{
						//当前部门的文件下载地址存入数据库
						$sql3 = "insert into xsrb_excel(`createtime`,`dept_id`,`biao`,`date`,`url`) values(now(),".$vde['id'].",'fdmkcbmx',$date,'$keys')";
						M()->execute($sql3);
					}
				}
				$ret = -1;
