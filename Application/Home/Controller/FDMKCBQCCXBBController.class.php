...
class FDMKCBQCCXBBController extends RestController
{
    ...
    public function search($token ='',$date ='',$page ='',$pagesize ='',$type ='',$id ='')
    {
		...
		$handle1=fopen(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/qc_json1.txt"),'r');
		$qc_json1=fread($handle1, filesize(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/qc_json1.txt")));
		fclose($handle1);
		$handle2=fopen(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/qc_json2.txt"),'r');
		$qc_json2=fread($handle2, filesize(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/qc_json2.txt")));
		fclose($handle2);			
        ...
        if($type =='excel')
        {
            ...
            foreach ($dept as $k1 => $v1)
            {
				$handle=fopen(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/qc_json3.txt"),'r');
				$qc_json3=fread($handle, filesize(str_replace('\\','/',realpath(__DIR__)."/tempJson/$banben/qc_json3.txt")));
				fclose($handle);
...
//				$xls = json_decode($revurl,true);
                ...
                if(!count($cxo))
				{
					...
					{
						//当前部门的文件下载地址存入数据库
						$sql = "insert into xsrb_excel(`createtime`,`dept_id`,`biao`,`date`,`url`) values(now(),".$vde['id'].",'fdmkcbqccxbb','".$date."','$keys')";
						M()->execute($sql);
					}				
				}
